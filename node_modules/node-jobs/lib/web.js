...
function startServer(store,port) {
	...
	var router = urlrouter(function (app) {
		...
		app.get('/jobs/export',function(req,res) {
			store.getJobs(function(data) {
				var page = "#name,id,start,end,status\n";
				data.forEach(function(job) {


					var str = [job.name,
								job.id,
								job.start !== 'undefined' ? new Date(job.start).toISOString() : new Date().toISOString(),
								job.end !== 'undefined' ? new Date(job.end).toISOString() : new Date().toISOString(),
								job.status];
					page += str.join(',') + '\n';
				});
				res.writeHead(200, {
