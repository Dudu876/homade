...
module.exports.init = function(redisclient,messageClient) {
	if(!redisclient) { throw 'redisClient is null'; }
	if(!messageClient) { throw 'messageClient is null'; }
		...
		client.hmset(key,reset,function(err) {
			...
			client.hget(key,'name',function(err,taskname) {
				client.lpush(rediskeys.PENDING_JOBS_KEY(taskname),jobId,function(err) {
					if(err) { return console.error(err); }
   					messageHandler.publishToJobQueue(taskname,'queue');
				});
			});
		});
	});
