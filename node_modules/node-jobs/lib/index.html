		...
		<div class='col-md-12'>
			<div class='row'>
				...
				<h2>Jobs
					...
					<a href='/jobs/export' target='_new' class='h4'>export</a>

				</h2>
				<label>Search</label>
				<input ng-init='search=""' ng-model='search' placeholder='search'/>
				<table class='table table-bordered table-striped table-hover' ng-init='predicate="endstr";reverse=true'>
					<thead>
						<tr>
							...
							<th><a href='javascript:void(0);' ng-click='predicate="startstr";reverse=!reverse'>Start</a></th>
							<th><a href='javascript:void(0);' ng-click='predicate="endstr";reverse=!reverse'>End</a></th>
						</tr>
					</thead>
					<tbody>
						<tr ng-repeat='job in model.jobs | filter:search | orderBy:predicate:reverse' ng-click='model.job=showJobDetails(job.id)' data-toggle='modal' data-target='#jobModal'>
							...
							<td ng-init='job.startstr = parseDate(job.start)'>{{job.start}}</td>
							<td ng-init='job.endstr = parseDate(job.end)'>{{job.end}}</td>
						</tr>
					</tbody>
				</table>
